...
void createNFA()
{
	...
	while(N--)
	{
		...
		if(strlen(str)>1)
		{
			mov[ch][str[0]].set[mov[ch][str[0]].len++]=str[1];
		}
		else
		{
			//if(str[0]=='@')        //用@表示伊布熊
				//is_final[ch]=true;
			//else
			mov[ch][str[0]].set[mov[ch][str[0]].len++]='Y';  //终态
		}
	}
}
void showNFA()
{
	for(int i=0;i<100;++i)
	{
		for(int j=0;j<100;++j)
		{
			...
			{
				if(mov[i][j].set[k]!='#')
					cout<<char(i)<<" "<<char(j)<<" "<<k<<" "<<mov[i][j].set[k]<<endl;
			}
		}
	}
}
...
void get_closure(NFA_set &temp)    //得到一个完整的子集 
{
	for(int i=0;i<temp.len;++i)
	{
		for(int j=0;j<mov[temp.set[i]]['@'].len;++j)
		{
			if(!Is_in_set(mov[temp.set[i]]['@'].set[j],temp))
			{
				temp.set[temp.len++]=mov[temp.set[i]]['@'].set[j];
			}
		}
	}
}
...
void NFA_to_DFA()
{
	...
	while(!s.empty())
	{
		...
		for(int i=0;i<len_final;++i)
		{
			...
			{
				for(int k=0;k<mov[work_set.set[j]][final[i]].len;++k)
				{
					if(mov[work_set.set[j]][final[i]].set[k]!='#' && mov[work_set.set[j]][final[i]].set[k]!='Y' && !Is_in_set(mov[work_set.set[j]][final[i]].set[k],worked_set))
					{
						worked_set.set[worked_set.len++]=mov[work_set.set[j]][final[i]].set[k];
					}
					if(mov[work_set.set[j]][final[i]].set[k]=='Y' && !Is_in_set(mov[work_set.set[j]][final[i]].set[k],worked_set))
					{
